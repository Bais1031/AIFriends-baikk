...
- **语音交互**: 支持语音输入（ASR）和语音输出（TTS），角色可自选音色
...
### 角色音色选择
- 创建/更新角色时可选音色，支持 11 种阿里云 CosyVoice-v3-flash 音色（5 男声 + 6 女声）
- 音色 ID 存储在 Character 模型 `speaker` 字段，默认 `longanyang`（向后兼容）
- 聊天时 TTS 自动读取角色音色，不同角色可使用不同声音
- 音色通过 WebSocket duplex 流式协议实时合成，无需改动 TTS 架构
