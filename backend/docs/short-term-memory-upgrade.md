...
| 维度 | 改进前 | 改进后 |
|---|---|---|
| **上下文结构** | SystemPrompt + 固定10条历史 + 当前消息 | SystemPrompt + 对话摘要 + 动态窗口历史 + 当前消息 |
| **消息窗口策略** | 固定取最近 10 条，无 token 感知 | Token 预算约束（2096 token），从最新往前动态取，超预算截断 |
| **早期上下文保留** | 超过 10 条后完全丢失 | 通过对话摘要层保留早期对话要点 |
| **对话摘要机制** | 无 | 每 20 条新消息触发 LLM 自动摘要，与已有摘要合并 |
| **Token 估算（中文）** | `len(text) // 2`（1 token ≈ 2 字，低估） | `中文字数 / 1.5`（1 token ≈ 1.5 字，更准确） |
| **Token 估算（英文）** | `len(text) // 2`（偏高） | `英文词数 × 1.3`（按词计算，更合理） |
| **Token 预算管理** | 无，可能超出模型上下文 | 4096 总预算，分区预留（系统 1500 / 摘要 500 / 消息 2096） |
| **Token 记录来源** | 仅 `TokenCache.estimate_tokens()` 粗估 | 优先 LLM `usage_metadata` 实际用量，回退估算 |
| **长期记忆更新频率** | `count % 1 == 0`（每条消息都触发，bug） | `count % 5 == 0`（每 5 条触发一次） |
| **多模态消息顺序** | 历史消息插在当前消息之后（bug） | 正确顺序：系统提示 → 摘要 → 历史 → 当前消息 |
| **图片描述位置** | 与历史消息混在一起 | 独立插入系统提示之后（位置 1） |
| **上下文组装方式** | 散落的 `add_system_prompt()` + `add_recent_messages()` | 统一的 `ContextBuilder.build()` 集中管理 |
| **模型字段** | `memory`（仅长期记忆） | 新增 `conversation_summary` + `summary_message_count` |
